...
class UdpServer {
    ...
    static constexpr int MAX_MSG_SIZE = 1200;
    static constexpr int MAX_INPUTS_PER_TICK = 4;

    struct ClientSession {
        std::unique_ptr<ReliableChannel> channel;
        sockaddr_storage addr;
        int inputCount = 0;     // inputs this tick (rate limiting)
        int reconnectPid = -1;  // player_id to reconnect to
    };
    std::unordered_map<uint32_t, ClientSession> sessions_;

public:
    UdpServer() { gameTick_.data = this; cleanupTick_.data = this; pingTick_.data = this; }
    ~UdpServer() { stop(); }
    ...
    bool start(int port) {
        ...
        if (!socket_.startRecv([this](const std::string& data, const sockaddr_storage& addr) {
            // Message size limit
            if (data.size() > MAX_MSG_SIZE) return;
            onRecv(data, addr);
        })) return false;

        // Game tick timer (33ms = 30 Hz)
        uv_timer_init(loop_, &gameTick_);
        uv_timer_start(&gameTick_, [](uv_timer_t* t) {
            auto* s = (UdpServer*)t->data;
            // Reset per-tick input counters
            for (auto& [cid, ses] : s->sessions_) ses.inputCount = 0;
            s->onGameTick();
        }, 33, 33);
        ...
        // Ping timer (3 seconds)
        uv_timer_init(loop_, &pingTick_);
        uv_timer_start(&pingTick_, [](uv_timer_t* t) {
            ((UdpServer*)t->data)->sendPingAll();
        }, 3000, 3000);
    ...
    void runAsync() { if (loop_ && running_) uv_run(loop_, UV_RUN_NOWAIT); }
    void stop() {
        running_ = false;
        uv_timer_stop(&gameTick_); uv_timer_stop(&cleanupTick_); uv_timer_stop(&pingTick_);
        socket_.close();
...
private:
    void onRecv(const std::string& data, const sockaddr_storage& addr) {
        ...
        if (!conn) {
            ...
            if ((int)sessions_.size() >= MAX_CONNECTIONS) return;
            ...
            auto ch = std::make_unique<ReliableChannel>(
                [this, addr](const std::vector<uint8_t>& pkt) {
                    socket_.sendTo((const char*)pkt.data(), pkt.size(), (const struct sockaddr*)&addr);
                });
    ...
    void handleMessage(uint32_t cid, uint8_t msgType, const std::string& content) {
        ...
        // Input rate limiting (keyDown, keyUp, joyAxis, joyHat)
        if (msgType >= 3 && msgType <= 6) {
            if (++session.inputCount > MAX_INPUTS_PER_TICK) return;
        }
        ...
        if (msgType == 0) {  // connect
            if (game_->currState() == GameState::mainMenu) {
                ...
                sendToClient(cid, Message("server", "connect", {{"player_id", pid}}));
            } else {
                sendToClient(cid, Message("server", "connect", {{"player_id", -1}}));
            }
        }
        else if (msgType == 1) {  // disconnect
            auto dp = game_->board.findPlayer(conn->playerId);
            if (dp) { dp->isAlive = false; }
            connMgr_.remove(cid); sessions_.erase(cid);
        }
            ...
            if (p) {
                if (msgType == 3) {
                    if (key == Keys::p) {
                        if (!game_->isPaused) {
                            game_->isPaused = true; game_->pausePlayerId = p->player_id;
                            game_->pausePlayerName = p->name;
                        } else if (game_->pausePlayerId == p->player_id) {
                            game_->isPaused = false; game_->pausePlayerId = -1;
            ...
            if (content.size() >= 5) {
                int axis = (uint8_t)content[0]; float value;
                memcpy(&value, content.data() + 1, 4);
                auto p = game_->board.findPlayer(conn->playerId);
                if (p) {
                    if ((int)p->joystickAxisList.size() <= axis) p->joystickAxisList.resize(axis + 1);
            ...
            if (content.size() >= 2) {
                int x = (int8_t)content[0], y = (int8_t)content[1];
                auto p = game_->board.findPlayer(conn->playerId);
                if (p) {
                    auto& kl = p->pressedKeyList;
                    auto rm = [&](int k) { kl.erase(std::remove(kl.begin(), kl.end(), k), kl.end()); };
                    rm(Keys::w); rm(Keys::s); rm(Keys::a); rm(Keys::d);
                    if (x == 1) kl.push_back(Keys::d); if (x == -1) kl.push_back(Keys::a);
                    if (y == 1) kl.push_back(Keys::w); if (y == -1) kl.push_back(Keys::s);
                }
            }
        }
        else if (msgType == 7) {  // pong (response to server ping)
            // Server tracks lastRecvTime via updateRecvTime; no further action needed
        }
        else if (msgType == 8) {  // reconnect
            int pid = content.empty() ? -1 : (int)(uint8_t)content[0];
            auto existing = game_->board.findPlayer(pid);
            if (existing && !existing->isAlive) {
                // Re-activate disconnected player
                existing->isAlive = true;
                conn->playerId = pid;
                session.reconnectPid = pid;
                sendToClient(cid, Message("server", "connect", {{"player_id", pid}}));
            } else {
                sendToClient(cid, Message("server", "connect", {{"player_id", -1}}));
            }
    ...
    void onGameTick() {
        if (!game_) return;
        if (!game_->isPaused) game_->update(); else game_->getObjects();
    ...
    void sendPingAll() {
        ...
        for (auto& [cid, session] : sessions_) {
            // Send ping (msgType=7, empty content)
            std::vector<uint8_t> data;
            data.push_back(7);  // ping type
            // Wrap in length-prefixed format
            std::vector<uint8_t> pkt;
            pkt.push_back((uint8_t)data.size());
            pkt.insert(pkt.end(), data.begin(), data.end());
            socket_.sendTo((const char*)pkt.data(), pkt.size(), (const struct sockaddr*)&session.addr);
        }
    }

    void broadcastGameState() {
        if (!msgQueue_ || sessions_.empty()) { if (msgQueue_) msgQueue_->clear(); return; }
        Message msg;
        while (msgQueue_->tryPop(msg)) {
            auto bytes = serializeMessage(msg);
            for (auto& [cid, session] : sessions_) {
                socket_.sendTo((const char*)bytes.data(), bytes.size(), (const struct sockaddr*)&session.addr);
            }
        }
    }
    ...
    void sendToClient(uint32_t cid, const Message& msg) {
        ...
        auto bytes = serializeMessage(msg);
        socket_.sendTo((const char*)bytes.data(), bytes.size(), (const struct sockaddr*)&it->second.addr);
    }

    std::vector<uint8_t> serializeMessage(const Message& msg) {
        std::string json = msg.str();
        if (json.size() > MAX_MSG_SIZE - 1) json = json.substr(0, MAX_MSG_SIZE - 1);
        std::vector<uint8_t> data;
